...
# 你看看我这个extract脚本 我从lerobot格式的数据集里提取图片特征。能不能也用一个类似的脚本调用V3R_pi0里的multimodal spatial encoder里的cut3r encoder把可视化的点云渲染出来 我想看看cut3r效果到底怎么样？我的要求如下：第一,不要给我写很多很复杂的文档。第二,我觉得可以直接用cut3r spatial encoder文件里的  ```python export_point_cloud ```  函数,能用现有的函数就用现有的函数。其次我希望额外创建一个脚本。用我在extract feature调用的utils/spatiotemporal lerobot dataset,按照extract feature一样的流程,只不过用export point cloud把点云渲染出来,,支持debug_episode函数,然后创建一个视频或者别的让我来看。由于我使用的时候是把同一时刻的两个图像压缩到Batch里了,能不能一个周期创建两个视频,一个是base视角一个是wrist视角。
